...
export class RegistroUsuariosScreenComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    // Obtener de la URL el rol para saber cuál editar
    if(this.activatedRoute.snapshot.params['rol'] != undefined){
      this.rol = this.activatedRoute.snapshot.params['rol'];
      console.log("Rol detectado: ", this.rol);
      // Preseleccionar el tipo de usuario y banderas para edición
      this.user.tipo_usuario = this.rol;
      this.tipo_user = this.rol;
      if (this.rol === 'administrador') {
        this.isAdmin = true; this.isAlumno = false; this.isMaestro = false;
      } else if (this.rol === 'alumno') {
        this.isAdmin = false; this.isAlumno = true; this.isMaestro = false;
      } else if (this.rol === 'maestro') {
        this.isAdmin = false; this.isAlumno = false; this.isMaestro = true;
      }
    }

    // Validar si existe un parámetro ID en la URL
  ...
  public obtenerUserByID() {
    ...
    if(this.rol === 'administrador'){
      ...
      this.maestrosService.obtenerMaestro(this.idUser).subscribe(
        (response: any) => {
          this.user = response;
          this.user.first_name = response.user?.first_name || response.first_name;
          this.user.last_name = response.user?.last_name || response.last_name;
      ...
      this.alumnosService.obtenerAlumno(this.idUser).subscribe(
        (response: any) => {
          this.user = response;
          this.user.first_name = response.user?.first_name || response.first_name;
          this.user.last_name = response.user?.last_name || response.last_name;
